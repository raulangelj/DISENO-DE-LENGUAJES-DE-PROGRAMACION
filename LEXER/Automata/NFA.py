...
class NFA(Automata):
    ...
    def create_automata(self, postfix: str, infix: str = 'NFA', original: str = '') -> Automata:
        ...
        for caracter in postfix:
            if caracter == self.simbols.concat.get_simbol():
                automata_concat = self.simbols.concat.get_automata_rule(
                    self.automata_stack[-2], self.automata_stack[-1])
                self.add_to_automata_stack(automata_concat)
            elif caracter == self.simbols.kleen.get_simbol():
                automata_kleen = self.simbols.kleen.get_automata_rule(
                    self.automata_stack[-1], state_counter=self.get_state_counter())
                self.automata_stack.pop()
                self.automata_stack.append(automata_kleen)
                self.state_counter += 1
            elif caracter == self.simbols.or_op.get_simbol():
                automata_or = self.simbols.or_op.get_automata_rule(
                    self.automata_stack[-2], self.automata_stack[-1], state_counter=self.get_state_counter())
                self.add_to_automata_stack(automata_or)
                self.state_counter += 1
            else:
                self.automata_stack.append(self.create_token_automata(
                    caracter, self.get_state_counter()))
                self.state_counter += 1
    ...
    def simulate(self, input_string: str) -> str:
        ...
        for caracter in input_string:
            current_states = self.e_closure(
                self.move(current_states, Token(caracter)))
    ...
    def generate_state_from_Dstates(self, Dstates: List[List[State]], state_number: int, adf: DFA):
        ...
        new_state = State(value=state_number,
                          is_initial=state_number == 0, is_final=is_final)
    ...
    def DFA_subsets(self):
        ...
        for state in Dstates:
            for simbol in self.alphabet:
                ...
                if U != []:
                    Dtran.append(
                        [Dstates.index(state), simbol, Dstates.index(U)])
        # set the transitions and create the states
        for initial_state, token, final_state in Dtran:
            initial_state = self.generate_state_from_Dstates(
                Dstates, initial_state, dfa)

            final_state = self.generate_state_from_Dstates(
                Dstates, final_state, dfa)
