    ...
    def evaluate_rules(self, expression: List[Character], concat=True, already_evaluated=False):
        ...
        while index < len(expression):
            ...
            if caracter is self.kleen.get_simbol():
                ...
                or_operator, move, * \
                    forward = self.or_op.validate(factors, index, expression)
                if len(forward) > 1:
                    new_agrupation_evaluated = self.evaluate_rules(
                        or_operator, False)
                    or_operator, move, *forward = self.or_op.parse(
                        new_agrupation_evaluated, positions_move=move, caracter_before=forward[1])
                ...
                optional_operator, move = self.optional.validate(
                    factors, index)
                del factors[-move:]
    ...
    def remove_special_characters(self, expression: List[Character]) -> List[Character]:
        ...
        while index < len(expression):
            ...
            if caracter == self.plus.get_simbol():
                plus_operator, move_back = self.plus.evaluate(
                    new_expresion, index)
                del new_expresion[-move_back:]
                new_expresion += plus_operator
            elif caracter == self.optional.get_simbol():
                optional_operator, move_back = self.optional.evaluate(
                    new_expresion, index)
                del new_expresion[-move_back:]
