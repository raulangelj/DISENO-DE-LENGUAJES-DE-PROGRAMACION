...
class Optional(Operator):
	...
	def evaluate(self, factors: List[Character], index: int) -> List[Character]:
		...
		or_operator = Character(value=self.or_simbol, type=character_types.OPERATOR)
		...
		if not factors or caracter_before == self.or_simbol:
			...
			new_factors = [open_agrupation] + agrupation_optional + [or_operator, empty, close_agrupation]
			return new_factors, position_move
		else:
			# no hay parentesis quiere decir que es algo como a?
			new_factors = [open_agrupation, factors[-1], or_operator, empty, close_agrupation]
