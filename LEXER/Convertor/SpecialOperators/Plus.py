...
class Plus(Operator):
	...
	def evaluate(self, factors: List[Character], index: int) -> List[Character]:
		...
		new_factors = []
		open_agrupation = Character(value=self.agrupation[0], type=character_types.AGRUPATION)
		close_agrupation = Character(value=self.agrupation[1], type=character_types.AGRUPATION)
		concatenation = Character(value=self.concatenation_simbol, type=character_types.OPERATOR)
		kleene = Character(value=self.kleene_simbol, type=character_types.OPERATOR)
		...
		if not factors or caracter_before is self.union_simbol:
			...
			new_factors = [open_agrupation] + plus_factors + [concatenation] + plus_factors + [kleene] + [close_agrupation]
			return new_factors, move_back
		else:
			# no hay parentesis quiere decir que es algo como a+
			new_factors = [open_agrupation] + factors[-1:] + [concatenation] + factors[-1:] + [kleene] + [close_agrupation]
	...
	def validate(self, factors, index):  # sourcery skip: raise-specific-error
		...
		if len(factors) == 0 or caracter_before is self.union_simbol:
			...
			agrupation_plus, position_move = self.find_agrupation("".join(factors))
