...
class Automata():
    ...
    def create_token_automata(self, value: str, state_counter: int) -> Automata:
        ...
        token_automata.states: List[State] = [
            token_automata.initial_state, token_automata.final_state]
        token_automata.alphabet: List[Token] = [token]
        token_automata.transitions.add_transition(
            initial_state, token, final_state)
    ...
    def add_state(self, state: State) -> None:
        is_in = any(state_item.value ==
                    state.value for state_item in self.states)
        if not is_in:
    ...
    def create_graph(self, fileName: str = 'Automata', ) -> None:
        ...
        for state in self.states:
            if state.is_final:
                dot.node(str(state.value), str(
                    state.value), shape='doublecircle')
            else:
        ...
        for state_1, token, state_2 in self.transitions.transitions:
            dot.edge(str(state_1.value), str(
                state_2.value), label=str(token.value))
