...
class Or(Operator):
    ...
    def get_automata_rule(self, operator1: Automata, operator2: Automata, state_counter: int):
        ...
        automata = Automata()
        # Add the new states
        automata.states = operator1.states + \
            operator2.states + [new_first_state, new_last_state]
        # Add the alphabet
        automata.add_alphabet(operator1.alphabet +
                              operator2.alphabet + [EmptyToken()])
        # Add the transitions
        ...
        automata.transitions.add_transition(
            new_first_state, EmptyToken(), operator1.initial_state)
        automata.transitions.add_transition(
            new_first_state, EmptyToken(), operator2.initial_state)
        automata.transitions.add_transition(
            operator1.final_state, EmptyToken(), new_last_state)
        automata.transitions.add_transition(
            operator2.final_state, EmptyToken(), new_last_state)
